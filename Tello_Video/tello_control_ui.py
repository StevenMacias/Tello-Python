...
class TelloUI:
    ...
    def __init__(self,tello,outputpath):
        ...
        self.stopEvent = None
        self.status_data = {'bat': 0.0,'agy': 0.0,'time': 0.0,'h': 0.0,'yaw': 0.0,'temph': 0.0,'templ': 0.0,'vgx': 0.0,'vgy': 0.0,'vgz': 0.0,'pitch': 0.0,'agx': 0.0,'tof': 0.0,'roll': 0.0,'baro': 0.0,'agz': 0.0}
        ...
        self.status_thread = threading.Thread(target = self._statusPlots)
    def videoLoop(self):
        """
    ...
    def _statusPlots(self):
        f = Figure(figsize=(5,4), dpi=100)
        ax = f.add_subplot(111)
        canvas = FigureCanvasTkAgg(f, master=self.root)
        canvas.get_tk_widget().pack(side=Tk.TOP, fill=Tk.BOTH, expand=1)
        while True:
            data = self.tello.get_status()
            print(data)
            data = {"pitch" : data["pitch"], "roll" : data["roll"], "yaw":data["yaw"]}
            names = list(data.keys())
            values = list(data.values())
            rects1 = ax.bar(range(len(data)),values,tick_label=names)
            canvas.draw()
            time.sleep(0.1)
            ax.clear()
    ...
    def openCmdWindow(self):
        ...
        self.distance_bar = Scale(panel, from_=0.2, to=1, tickinterval=0.2, digits=3, label='Distance(m)',
                                  resolution=0.1)
